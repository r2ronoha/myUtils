...
if [[ `docker ps &> /dev/null; echo $?` -eq 0 ]]; then
	...
	if [[ `docker ps | egrep "venya-mongodb|venya-node-server" &> /dev/null; echo $?` -ne 0 ]]; then
		echo ""
		echo "WARNING: Venya staging not running. Starting it"
		echo "----------------------------------------------------"
		$scriptsdir/$venyaStagingStart -r
	else
		echo ""
		echo "** Venya staging environment already running **"
		echo ""
	fi
