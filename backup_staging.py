...
## VARIABLES
#basedir = os.path.join("/mnt","c","Users","arturo","OneDrive","VenYa","SOURCES","DOCKER_CONTAINERS","STAGING").replace('\\','/'.replace('\\','/')
stagingpath = os.path.join("c","Users","arturo","OneDrive","VenYa","SOURCES","DOCKER_CONTAINERS","STAGING").replace('\\','/')
...
if __name__ == "__main__":
	...
	#identify root folder /mnt or /c to set base dir
	root = run("pwd | cut -d'/' -f2")
	print "root folder = {}".format(root)
	if root == "c":
		basedir = os.path.join("/",stagingpath).replace('\\','/')
	else:
		basedir = os.path.join("/",root,stagingpath).replace('\\','/')
